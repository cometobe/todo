...
app.config['SQLALCHEMY_DATABASE_URI'] = os.getenv('DATABASE_URL', 'sqlite:///todolist.db')
...
class Item(db.Model):
    ...
    need_complete_date =db.Column(db.String(128))
    complete_date = db.Column(db.String(128))
    category_id = db.Column(db.Integer, db.ForeignKey('category.id'), default=1)
    # def __init__(self,body,category,need_complete_date,complete_date):
    #     self.create_date = datetime.now().strftime("%Y-%m-%d %H:%M:%S")
    #     self.body=body
    #     self.category=category
    #     self.need_complete_date=need_complete_date
...
#     """
    ...
    # db.session.add_all([inbox, done,work])
    # db.session.commit()
...
def index():
    if request.method == 'POST':
        ...
        need_complete_date = request.form.get('need_complete_date')
        category = Category.query.get_or_404(category_id)
        create_date = datetime.now().strftime("%Y-%m-%d")
        item = Item(body=body, category=category,need_complete_date=need_complete_date,create_date=create_date)
        db.session.add(item)
        db.session.commit()
...
def category(id):
    ...
    len = Item.query.filter(Item.category_id != 2).count()
    if id ==1:
        items= Item.query.filter(Item.category_id  != 2).all()
    return render_template('index.html', items=items,
                           categories=categories, category_now=category,len=len)
...
def edit_item(id):
    ...
    item.need_complete_date = request.form.get('need_complete_date')
    db.session.add(item)
    db.session.commit()
    return redirect(url_for('category', id=category.id))
...
def done(id):
    ...
    need_complete_date=item.need_complete_date
    create_date=item.create_date
    done_item = Item(body=item.body, category=done_category,complete_date=complete_date,create_date=create_date,need_complete_date=need_complete_date)
